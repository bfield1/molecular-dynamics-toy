...
def test_preset_positions_within_cell(preset_id):
    ...
    assert np.all(positions >= -1e-6), f"{preset_id}: Some positions are negative"
    assert np.all(positions <= cell_size + 1e-6), f"{preset_id}: Some positions exceed cell size"
