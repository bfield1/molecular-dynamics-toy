...
def test_periodic_table_single_selection(pygame_init):
    ...
    # Click H
    event = pygame.event.Event(pygame.MOUSEBUTTONDOWN, {'button': 1, 'pos': h_button.rect.center})
    ...
    # Click C
    event = pygame.event.Event(pygame.MOUSEBUTTONDOWN, {'button': 1, 'pos': c_button.rect.center})
    widget.handle_event(event)
...
def test_periodic_table_resize(pygame_init):
    ...
    assert len(widget.buttons) == initial_button_count  # Same number of buttons
